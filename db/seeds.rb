...
result.each do |park|
    p = Park.create(name: park['fullName'], description: park['description'], latitude: park['latitude'], longitude: park['longitude'], states: park['states'], directions: park['directionsInfo'], weatherInfo: park['weatherInfo'])
    ...
    # hoursArray = park['operating_hours']
    # hoursHash = hoursArray [0]
    # p.operatingHours.create(description: hoursHash['description'])
end
